...
class CAS(metric_base.MetricBase):
    def __init__(self, num_images, minibatch_per_gpu, img_size=[224, 224], train_sz=10000, val_sz=10000, **kwargs):
    ...
    def _get_partitioned_dataset_obj(self):
        ...
        self._train_dataset_obj = dataset.load_dataset(data_dir=self._data_dir, **dict(self._dataset_args, max_images=self.train_sz))
        self._val_dataset_obj = dataset.load_dataset(data_dir=self._data_dir, **dict(self._dataset_args, skip_images=self.train_sz, max_images=self.val_sz))
